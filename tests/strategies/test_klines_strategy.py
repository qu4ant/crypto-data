"""
Tests for KlinesStrategy (spot and futures OHLCV data).
...
from crypto_data.strategies import KlinesStrategy
...
class TestKlinesStrategyInit:
    ...
    def test_futures_strategy(self):
        """Create a futures klines strategy."""
    ...
    def test_invalid_data_type_raises(self):
        ...
        with pytest.raises(ValueError, match="only supports SPOT or FUTURES"):
            KlinesStrategy(DataType.FUNDING_RATES, Interval.MIN_5)

    def test_requires_interval_returns_true(self):
        """requires_interval() returns True for klines strategy."""
        strategy = KlinesStrategy(DataType.SPOT, Interval.MIN_5)
        assert strategy.requires_interval() is True

    def test_is_monthly_true(self):
        """is_monthly property returns True for klines strategy."""
        strategy = KlinesStrategy(DataType.SPOT, Interval.MIN_5)
        assert strategy.is_monthly is True

    def test_default_max_concurrent_20(self):
        """default_max_concurrent returns 20 for klines strategy."""
        strategy = KlinesStrategy(DataType.SPOT, Interval.MIN_5)
        assert strategy.default_max_concurrent == 20


class TestKlinesStrategyPeriods:
    """Tests for KlinesStrategy period generation."""
...
class TestKlinesStrategyUrls:
    """Tests for KlinesStrategy URL building."""
    ...
    def test_spot_url(self):
        ...
        expected = (
            ...
            'data/spot/monthly/klines/BTCUSDT/5m/BTCUSDT-5m-2024-01.zip'
        )
        assert url == expected
...
class TestKlinesStrategyCsvParsing:
    """Tests for KlinesStrategy CSV parsing."""
    ...
    def test_parse_csv_with_header(self):
        ...
        try:
            df = strategy.parse_csv(temp_path, 'ETHUSDT', 'binance')
...
class TestKlinesStrategySchema:
    """Tests for KlinesStrategy schema."""

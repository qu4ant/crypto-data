...
from crypto_data.universe_filters import DEFAULT_UNIVERSE_EXCLUDE_SYMBOLS, DEFAULT_UNIVERSE_EXCLUDE_TAGS
...
def test_create_binance_database_orchestrates_full_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch('crypto_data.database_builder.update_coinmarketcap_universe', new_callable=AsyncMock) as mock_update_coinmarketcap_universe, \
             patch('crypto_data.database_builder.get_binance_symbols_from_universe') as mock_get_symbols, \
...
def test_create_binance_database_uses_default_data_types():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Mock functions
        with patch('crypto_data.database_builder.update_coinmarketcap_universe', new_callable=AsyncMock), \
             patch('crypto_data.database_builder.get_binance_symbols_from_universe') as mock_get_symbols, \
             patch('crypto_data.binance_pipeline.update_binance_market_data') as mock_ingest_binance:

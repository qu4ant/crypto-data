"""
Klines Strategy for Spot and Futures OHLCV Data

Implements the DataTypeStrategy ABC for downloading and parsing
kline (candlestick) data from Binance spot and futures markets.
"""
...
class KlinesStrategy(DataTypeStrategy):
    """
    Strategy for downloading and parsing klines (OHLCV) data.
    ...
    def __init__(self, data_type: DataType, interval: Interval) -> None:
        """Initialize the klines strategy."""
        if data_type not in (DataType.SPOT, DataType.FUTURES):
            raise ValueError(
                f"KlinesStrategy only supports SPOT or FUTURES, got {data_type}"
            )
    ...
    def requires_interval(self) -> bool:
        """Return True - klines data requires an interval parameter."""
        return True

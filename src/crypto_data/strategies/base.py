...
Defines the abstract base class and data structures for data type strategies.
Each concrete strategy (klines, open_interest, funding_rates) implements these
interfaces to provide pluggable download and import logic.
"""
...
class DataTypeStrategy(ABC):
    ...
    Each data type (klines, open_interest, funding_rates) implements this
    interface to provide pluggable download and import logic.
    ...
    def requires_interval(self) -> bool:
        """
        Return True if this data type requires an interval parameter.

        Default implementation returns False. Klines strategies should override
        this to return True.

        Returns
        -------
        bool
            True if interval is required, False otherwise
        """
        return False

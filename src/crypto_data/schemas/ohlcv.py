...
OHLCV_SCHEMA = DataFrameSchema(
    columns={
        ...
        'exchange': Column(
            str,
            checks=[
                Check.isin(['binance']),  # Currently only Binance supported
                Check.str_length(min_value=1)
            ],
            nullable=False,
            description="Exchange name (currently 'binance')"
        ),
        'symbol': Column(
            str,
            checks=[
        ...
        'interval': Column(
            str,
            checks=[
                Check.isin(['1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d', '3d', '1w', '1M'])
            ],
            nullable=False,
        ...
        'trades_count': Column(
            ...
            checks=[
                Check.greater_than_or_equal_to(0, error="Trades count cannot be negative")
            ],
            nullable=True,
